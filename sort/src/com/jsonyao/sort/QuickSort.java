...
public class QuickSort {
    ...
    public int[] quickSort1(int[] arr, int left, int right){
        /**
         *  左右指针法1:(推荐, 对比第2种写法, 由于是只有满足条件时才执行交换, 大大减少了交换的次数, 但是却带来了强顺序性, 导致无法更改pivot的取值方式, 所以pivot固定追求性能时可以使用)
         *  A. 核心思想:
...
//        return arr;
    ...
    public int[] quickSort2(int[] arr, int left, int right){
        /**
         * 挖坑法(推荐, 实现上和性能上有点类似于第1种的左右指针法, 但思路比左右指针法更加形象, 但却带来了强顺序性, 导致piovt取值不能更改, 所以在pivot固定追求性能时可以使用):
         * A. 核心思想:
